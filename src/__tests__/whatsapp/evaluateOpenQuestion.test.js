...
vi.mock("@/lib/services/openaiResponses.service", () => ({ resolveAssistantVectorStore: vi.fn() }));
...
const evaluation = { verdict: "completa", missing_points: [], feedback: "Certo, os pneus devem estar frios." };
function setup(response = { status: "completed", output_text: JSON.stringify(evaluation) }) {
  const create = vi.fn(async () => response);
  return { assistant: { id: 7, model: "gpt-5.6", instructions: "Sê breve.", vector_store_id: 2 },
    question: { body: "Como verificas?", expected_answer: "A frio." }, answerText: "A frio.",
    deps: { openai: { responses: { create } }, resolveAssistantVectorStore: vi.fn(async () => "vs_123") } };
}
describe("avaliação isolada", () => {
  it("usa modelo, instruções, ficheiros e esquema JSON sem conversa", async () => {
    ...
    expect(request.tools).toEqual([{ type: "file_search", vector_store_ids: ["vs_123"] }]);
  ...
  it.each([
    ...
    { status: "completed", output_text: JSON.stringify({ ...evaluation, verdict: "certa" }) },
    { status: "completed", output_text: JSON.stringify({ ...evaluation, feedback: "" }) },
    { status: "completed", output_text: JSON.stringify({ ...evaluation, missing_points: [42] }) },
    { status: "completed", output_text: JSON.stringify({ ...evaluation, feedback: "a".repeat(1025) }) },
  ...
  it("funciona sem base de conhecimento", async () => {
    ...
    expect(args.deps.openai.responses.create.mock.calls[0][0].tools).toBeUndefined();
  });
  it("guarda pergunta, resposta e feedback por ordem sem gerar resposta", async () => {
    const create = vi.fn();
    await appendQuestionContext({ conversationId: "conv_1", question: { body: "Pergunta" }, answerText: "Resposta", replies: ["Feedback", "Esperada"], deps: { openai: { conversations: { items: { create } } } } });
    expect(create.mock.calls[0].slice(0, 2)).toEqual(["conv_1", { items: [
