...
describe("resposta a pergunta aberta", () => {
  ...
  beforeEach(() => {
    ...
    const message = {
      ...
      user_id: 5700,
      question_id: 10,
      content: "João, como verificas os pneus?",
    };
    deps = {
      ...
      evaluateOpenQuestion: vi.fn(async () => ({
        verdict: "parcial",
        missing_points: ["Temperatura"],
        feedback: "Falta indicar a temperatura dos pneus.",
      })),
      appendQuestionContext: vi.fn(),
      updateQuestionAnswerEvaluation: vi.fn(),
    };
    args = {
      user,
      payload: { body: { text: { text: "Com um manómetro" } } },
      deps,
  ...
  it("reserva a primeira resposta, avalia e envia feedback antes da resposta esperada", async () => {
    ...
    expect(deps.createQuestionAnswer.mock.invocationCallOrder[0]).toBeLessThan(
      deps.evaluateOpenQuestion.mock.invocationCallOrder[0],
    );
    expect(args.sendText.mock.calls.map(([text]) => text)).toEqual([
      "Falta indicar a temperatura dos pneus.",
      "Resposta esperada:\nCom os pneus frios.",
    ]);
    expect(deps.evaluateOpenQuestion.mock.calls[0][0].question.body).toBe(
      "João, como verificas os pneus?",
    );
    expect(deps.appendQuestionContext).toHaveBeenCalledWith(
    ...
    expect(deps.updateQuestionAnswerEvaluation).toHaveBeenCalledWith(30, {
      verdict: "parcial",
      aiFeedback: "Falta indicar a temperatura dos pneus.",
      reviewNeeded: false,
    });
  });
  it("envia só a resposta esperada e marca revisão se a IA falhar", async () => {
    ...
    expect(await handleQuestionReply(args)).toMatchObject({
      reviewNeeded: true,
      verdict: null,
    });
    expect(args.sendText.mock.calls).toEqual([
      ["Resposta esperada:\nCom os pneus frios."],
    ]);
  });
  it("não avalia quando a IA está desligada", async () => {
    question.ai_evaluation = false;
    expect(await handleQuestionReply(args)).toMatchObject({
      reviewNeeded: false,
      verdict: null,
    });
  ...
  it("não envia uma repetição do mesmo evento Bird para o assistente", async () => {
    ...
    deps.getQuestionAnswer.mockResolvedValue({
      inbound_message_id: "bird-in-1",
    });
    deps.createQuestionAnswer.mockResolvedValue(null);
    expect(await handleQuestionReply(args)).toMatchObject({
      handled: true,
  ...
  it("tenta enviar a resposta esperada mesmo se o envio do feedback lançar erro", async () => {
    ...
    expect(args.sendText).toHaveBeenLastCalledWith(
      "Resposta esperada:\nCom os pneus frios.",
    );
  });
  ...
  it("não captura texto vazio, toques na abertura ou texto de pergunta expirada", async () => {
    ...
    args.payload.body.text = {
      text: "Aceito",
      actions: [{ type: "postback", postback: { payload: "item_0" } }],
    };
    expect(await handleQuestionReply(args)).toEqual({ handled: false });
  ...
  it("não associa a mensagem de outro contacto", async () => {
    ...
    deps.getMessageByProviderId.mockResolvedValue({
      user_id: 8,
      question_id: 11,
    });
