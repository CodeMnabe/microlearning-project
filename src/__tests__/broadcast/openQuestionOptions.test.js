...
describe("opções de pergunta aberta", () => {
  ...
  it("normaliza e liga a IA por omissão, incluindo payloads agendados", () => {
    expect(parseQuestionOptions({ question })).toEqual({
      question: {
        kind: "open",
        body: "Pergunta\ntexto",
        expectedAnswer: "Resposta",
    ...
    expect(
      parseQuestionOptions({ question: { ...question, aiEvaluation: false } })
  ...
  it.each([
    ...
    { expectedAnswer: " " },
    { body: "x".repeat(1025) },
    { expectedAnswer: "x".repeat(1025) },
    { aiEvaluation: "false" },
  ])("rejeita campos inválidos (%j)", (patch) => {
    expect(
      parseQuestionOptions({ question: { ...question, ...patch } }).error,
    ).toBeTruthy();
  });
});
