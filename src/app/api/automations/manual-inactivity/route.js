...
import { processInactivityRulesForOrganization } from "@/lib/services/automations/processInactivityRules";
...
async function enforceRateLimits(admin, orgId, userId, action) {
  ...
  const { data: orgData } = await admin.rpc("consume_request_capacity", { p_scope: "manual-inactivity-org", p_subject_hash: orgHash, p_window_seconds: 60, p_maximum_requests: 10 });
  ...
  const { data: actorData } = await admin.rpc("consume_request_capacity", { p_scope: "manual-inactivity-actor", p_subject_hash: actorHash, p_window_seconds: 60, p_maximum_requests: 5 });
  ...
  const { data: actionData } = await admin.rpc("consume_request_capacity", { p_scope: "manual-inactivity-action", p_subject_hash: actionHash, p_window_seconds: 60, p_maximum_requests: 1 });
...
export async function POST(req) {
  try {
    ...
    const rateLimit = await enforceRateLimits(auth.admin, orgId, auth.user.id, "inactivity");
    if (!rateLimit.ok) {
    ...
    const result = await processInactivityRulesForOrganization({ organizationId: orgId });
