import "server-only";
import { getActiveAutomationRulesGlobally, getActiveAutomationRulesForOrganization } from "@/lib/repos/automationRules.repo";
...
async function processRulesList(rules) {
  if (!rules.length) {
    return {
      ok: true,
...
export async function processInactivityRulesGlobally() {
  const rules = await getActiveAutomationRulesGlobally("user.inactive");
  return processRulesList(rules);
}

export async function processInactivityRulesForOrganization({ organizationId }) {
  if (!organizationId) throw new Error("organizationId is required");
  const rules = await getActiveAutomationRulesForOrganization(organizationId, "user.inactive");
  return processRulesList(rules);
}
